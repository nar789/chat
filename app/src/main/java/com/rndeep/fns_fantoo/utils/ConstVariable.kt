...
object ConstVariable {
    ...
    const val ERROR_COMMENT_IMAGE_UPLOAD_FAIL = "IM4010"
    ...
    class ClubDetail {
        companion object {
            ...
            const val KEY_JOIN_MEMBER_ID = "memberId"
        }
    }
    ...
    class ClubDef {
        companion object {
            ...
            const val KEY_CLUB_INFO_INTEREST_CATEGORY_CODE = "interestCategoryCode"
    ...
    class Community {
        companion object {
            const val MAX_REQ_POST_LIST_SIZE = 30
        }
        enum class CategoryCode(val code: String) {
            C_HOT("en_hot"),
            C_KPOP("en_k_pop"),
            C_KDRAMA("en_k_drama"),
            C_KMOVIE("en_k_movie"),
            C_SHOW("t_tv_show_variety"),
            C_GAME("g_game"),
            C_COMICS("m_comics"),
            C_BEAUTY("b_beauty_fashion"),
            C_SPORT("s_sports"),
            C_FOOD("a_food"),
            C_LIFE("a_life"),
            C_DISCUSSION("t_discussion"),
            C_NEWS("n_news");

            companion object {
                fun compare(text: String?): CategoryCode {
                    return try {
                        valueOf(text ?: "0")
                    } catch (e: Exception) {
                        C_HOT
        ...
        enum class HeaderTitleCode(val code: String) {
            TALK("j_any_chat"),
            REVIEW("r_review"),
            ASK("j_question"),
            NEWS("n_news"),
            ...
            companion object {
                fun compare(text: String?): HeaderTitleCode {
                    return try {
                        valueOf(text ?: "0")
                    } catch (e: Exception) {
                        TALK
