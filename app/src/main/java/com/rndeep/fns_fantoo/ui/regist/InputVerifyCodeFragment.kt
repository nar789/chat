...
@AndroidEntryPoint
class InputVerifyCodeFragment : RegistFragment<RegistInputVerifyCodeFragmentBinding>(RegistInputVerifyCodeFragmentBinding::inflate){
    ...
    override fun initUi(){
        ...
        initCountDownTimer()
        binding.tvTailDesc.text = String.format(getString(R.string.se_a_cert_number_will_sent), inputEmail)
        binding.tvResend.paintFlags = binding.tvResend.paintFlags or Paint.UNDERLINE_TEXT_FLAG

        inputVerifyCodeViewModel.checkingVerifyCodeLiveData.observe(viewLifecycleOwner){
            verifyCodeCheckResponse->
                verifyCodeCheckResponse?.let {
                    if(verifyCodeCheckResponse.isCheck){
                        ...
                        // 패스워드 설정화면으로 이동
                        cancelTimeCounter()
                        navController.navigate(R.id.action_inputVerifyCodeFragment_to_inputPasswordFragment, bundleOf(
    ...
    private fun initCountDownTimer() {
        cancelTimeCounter()
        binding.tvRemainTitle.setTextColor(ContextCompat.getColor(requireContext().applicationContext, R.color.primary_500))
        binding.tvRemainTime.setTextColor(ContextCompat.getColor(requireContext().applicationContext, R.color.primary_500))
        countDownTimer = object : CountDownTimer(LIMIT_TIME, 1000) {
            override fun onTick(millis: Long) {
                binding.tvRemainTime.text = String.format("%02d:%02d",
            ...
            override fun onFinish() {
                ...
                binding.tvRemainTitle.setTextColor(ContextCompat.getColor(context!!.applicationContext, R.color.gray_400))
                binding.tvRemainTime.setTextColor(ContextCompat.getColor(context!!.applicationContext, R.color.gray_400))
