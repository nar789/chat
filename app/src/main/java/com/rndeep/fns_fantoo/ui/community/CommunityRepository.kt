...
class CommunityRepository @Inject constructor(
    ...
    //커뮤니티 게시판 리스트
    suspend fun getCommunityBoardList(accessToken: String?,uId: String?): Pair<CommunityBoardCategoryBody?, ErrorBody?> {
        val safeRes = if (accessToken == null || uId == null) {
            safeApiCall(Dispatchers.IO) { communityService.getBoardListOfGuest() }
        } else {
            safeApiCall(Dispatchers.IO) { communityService.getBoardListOfMemberFantoo( accessToken,uId ) }
        }
        return when (safeRes) {
            is ResultWrapper.Success -> {
