...
class CommunityDetailPostViewModel @Inject constructor(
    ...
    private var modifyCommunityPost: ModifyCommunityPost? = null
    ...
    fun getCommunityPostData(boardCode: String, postId: Int) {
        viewModelScope.launch {
            ...
            if (detailResult.second != null) {
                ...
                replayNextId=0
                _postToolbarNameLiveData.value = communityPostData.code
                postData = listOf(
                ...
                val attachImage = arrayListOf<String>()
                postItem.attachList.forEach {
                    if(it.archiveType=="image"){
                        attachImage.add(it.id?:"")
                    }
                }
                attachImageList=attachImage
    ...
    fun requestCommunityNoticeDetailItem(boardCode: String?, postId: Int) {
        viewModelScope.launch {
            ...
            if (safeRes.second != null) {
                _detailPostErrorLiveData.value = safeRes.second!!.code?:ConstVariable.ERROR_WAIT_FOR_SECOND
            ...
            if (callRes.third != null) {
                ...
                if (postData != null) {
                    ...
                    callRes.first?.forEach {
                        if(it.activeStatus==2 && it.childReplyList.isNullOrEmpty()){
        ...
        viewModelScope.launch {
            ...
            if (isUser) {
                detailPostRepository.modifyComment(
                    ...
                    uId!!,
                    postId,
                    inputItem!!.commentString,
                    inputItem!!.anonymYN,
                    inputItem!!.attachType,
                    inputItem!!.attachUrl,
                    replyId
                ).run {
                    if (this == ConstVariable.RESULT_SUCCESS_CODE) {
    ...
    fun sendImageToCloudFlare(cloudFlareKey: String, file: MultipartBody.Part) {
        viewModelScope.launch {
            uploadRepository.fileUploadImageToCloudFlare(cloudFlareKey, file).run {
                if (this == null) {
                    _cloudFlareUploadLiveData.value = ConstVariable.ERROR_COMMENT_IMAGE_UPLOAD_FAIL
                } else {
    ...
    fun setModifyCommunityPost(
        postItem: DetailPostItem.CommunityPostDetail,
        postId: Int,
        boardCode: String
    ) {
        val attaches = if (postItem.attachItems.isEmpty()) {
            null
        } else {
            postItem.attachItems.map { list ->
                Attach(list.archiveType, list.id!!)
            }
        }

        val hashTag = if (postItem.hashTagItems.isEmpty()) {
            null
        } else {
            postItem.hashTagItems.map { list ->
                Hashtag(list.tagText)
            }
        }

        modifyCommunityPost = postItem.item.content?.let { content ->
            postItem.item.title?.let { title ->
                ModifyCommunityPost(
                    postItem.item.anonymYn,
                    attaches,
                    content,
                    hashTag,
                    postItem.item.integUid,
                    postItem.item.subCode,
                    title,
                    postId,
                    boardCode
                )
    ...
    fun getModifyCommunityPost() = modifyCommunityPost
