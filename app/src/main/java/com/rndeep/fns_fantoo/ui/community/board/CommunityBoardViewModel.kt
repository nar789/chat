...
class CommunityBoardViewModel @Inject constructor(
    ...
    suspend fun getLanguageCode()= dataStoreRepository.getString(DataStoreKey.PREF_KEY_SELECT_LANGUAGE_CODE)
    suspend fun setLanguageCode(langCode : String?){
        if(langCode==null){
            dataStoreRepository.clearString(DataStoreKey.PREF_KEY_SELECT_LANGUAGE_CODE)
            return
        }
        dataStoreRepository.putString(DataStoreKey.PREF_KEY_SELECT_LANGUAGE_CODE,langCode)
    }
