...
class ContentsFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.homeList.addOnScrollListener(object : RecyclerView.OnScrollListener() {
            override fun onScrollStateChanged(recyclerView: RecyclerView, newState: Int) {
                ...
                val isVisible = if (position > VISIBLE_ITEM_POSITION) View.VISIBLE else View.GONE
                binding.fab.visibility = isVisible
            }
        })
    ...
    private fun getContents() {
        lifecycleScope.launch {
            viewModel.fetchFantooClubHomePosts(viewModel.getClubId()).collectLatest {
    ...
    private fun showErrorMessage(msg: String) {
        val message = DialogMessage(
            DialogTitle(
                getString(R.string.p_fantoo_tv),
                msg,
                null
            ),
    ...
    private fun goToTop() {
        binding.homeList.scrollToPosition(TOP_ITEM_POSITION)
        binding.fab.visibility = View.GONE
    }

    companion object {
        private const val TOP_ITEM_POSITION = 0
        private const val VISIBLE_ITEM_POSITION = 3
    }
}