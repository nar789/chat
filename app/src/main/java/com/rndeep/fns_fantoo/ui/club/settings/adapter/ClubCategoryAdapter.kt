...
class ClubCategoryAdapter(val clickListener:(ClubInterestCategoryDto) -> Unit) : RecyclerView.Adapter<ClubCategoryAdapter.CategoryVH>(){
    private var selectedInterestCategoryCode : String = ""
    ...
    fun setSelectedInterestCategoryCode(interestCategoryCode:String){
        selectedInterestCategoryCode = interestCategoryCode
    }

    fun getSelectedInterestCategoryCode():String{
        return selectedInterestCategoryCode
    }
    ...
    inner class CategoryVH(val binding:TabClubSettingCategoryItemBinding): RecyclerView.ViewHolder(binding.root){
        ...
        fun bind(categoryInfo: ClubInterestCategoryDto){
            binding.tvCategoryName.text = categoryInfo.description
            binding.root.setOnClickListener {
                selectedInterestCategoryCode = categoryInfo.interestCategoryCode
                notifyDataSetChanged()
                clickListener(categoryInfo)
            }
            val isSelectedItem = (selectedInterestCategoryCode == categoryInfo.interestCategoryCode)
