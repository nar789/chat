...
class MultimediaAdapter(
    ...
    inner class ImageViewHolder(private val binding: ListItemEditorImageBinding) :
        ...
        fun bind(item: MultimediaItem) {
            ...
            binding.apply {
                val imageUrl = if (item.url?.contains("android.providers") == true) {
                    item.url
                } else {
                    binding.root.context.getString(R.string.imageUrlBase, item.url)
                }
    ...
    inner class VideoViewHolder(private val binding: ListItemEditorVideoBinding) :
        ...
        fun bind(item: MultimediaItem) {
            ...
            binding.apply {
                val videoThumbnailUrl = if (item.url?.contains("android.providers") == true) {
                    item.url
                } else {
                    binding.root.context.getString(R.string.videoThumbnail, item.url)
                }
                val option = MultiTransformation(CenterCrop(), RoundedCorners(24))
                Glide.with(root.context)
