...
class AgreeConfirmViewModel @Inject constructor(
        ...
        try {
            when (val response = registRepository.checkUserNickname(nickName)) {
                is ResultWrapper.Success -> {
                    _checkingNickNameLiveData.value = UserNickCheckResponse(RESULT_SUCCESS_CODE, "", response.data, null)
                }
                is ResultWrapper.GenericError -> {
                    _checkingNickNameLiveData.value =
                        UserNickCheckResponse(response.code.toString(), "", null, response.errorData)
                }
                else -> {}
            }
        }catch (e : Exception){
    ...
    fun joinBySns(value:HashMap<String,String>) = viewModelScope.launch {
        ...
        when(response){
            is ResultWrapper.Success ->{
                _joinSnsLiveData.value = RegistResponse(RESULT_SUCCESS_CODE, "", RegistResult(false), null)
            }
            is ResultWrapper.GenericError ->{
    ...
    fun getAuthCode(values:HashMap<String, String>) = viewModelScope.launch{
        ...
        when(response){
            is ResultWrapper.Success ->{
                _authCodeLiveData.value = AuthCodeResponse(RESULT_SUCCESS_CODE, "", response.data, null)
            }
            is ResultWrapper.GenericError->{
                _authCodeLiveData.value = AuthCodeResponse(response.code.toString(), response.message, null, response.errorData)
            }
    ...
    fun getAccessToken(values:HashMap<String, String>) = viewModelScope.launch {
        ...
        when(response){
            is ResultWrapper.Success ->{
                _accessTokenLiveData.value = AccessTokenResponse(RESULT_SUCCESS_CODE, "", response.data)
            }
            is ResultWrapper.GenericError ->{
                _accessTokenLiveData.value = AccessTokenResponse(response.code?.toString(),
