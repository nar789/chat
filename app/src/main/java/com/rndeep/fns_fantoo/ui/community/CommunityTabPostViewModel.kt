    ...
    fun getInitItem(){
        viewModelScope.launch {
            val boardRes =if(isUser()){
                val favoritySortType = dataStoreRepository.getString(DataStoreKey.PREF_KEY_COMMUNITY_FAVORITE_SORT_TYPE)
                repository.getCommunityBoardList(accessToken, uId, favoritySortType)
            }else{
                repository.getCommunityBoardList(null,null, null)
            }
