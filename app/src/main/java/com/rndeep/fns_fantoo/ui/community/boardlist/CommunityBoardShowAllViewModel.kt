...
class CommunityBoardShowAllViewModel @Inject constructor(
        ...
        viewModelScope.launch {
            ...
            boardCategoryData =
                if (!isUser) {
                    ...
                    dataStoreRepository.putString(DataStoreKey.PREF_KEY_COMMUNITY_FAVORITE_SORT_TYPE, type)
    ...
    suspend fun getFavoritySortType():String?{
        return dataStoreRepository.getString(DataStoreKey.PREF_KEY_COMMUNITY_FAVORITE_SORT_TYPE)
    }
    ...
    fun settingBottomSheetItem(bottomItemTitle: List<String>, bottomItemSubTitle: List<String>, checkedItem:List<Boolean>) {
        val bottomItems = ArrayList<BottomSheetItem>()
        for ((index, a) in bottomItemTitle.withIndex()) {
            bottomItems.add(BottomSheetItem(null, a, bottomItemSubTitle[index], checkedItem[index]))
        }
