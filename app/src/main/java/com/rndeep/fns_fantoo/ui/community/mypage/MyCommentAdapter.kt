...
class MyCommentAdapter : ListAdapter<MyCommentItem,RecyclerView.ViewHolder>(diff) {
    ...
    inner class MyCommentVH(
        ...
        fun bind(commentItem: MyCommentItem) {
            ...
            //프로필 이미지
            setProfileAvatar(binding.ivProfileThumbnail,itemView.context.getString(R.string.imageUrlBase,commentItem.userPhoto))
            //닉네임
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        if(viewType == EMPTY_VIEW){
            val view =LayoutInflater.from(parent.context)
             ...
             return CommunityNoListVH(CommunityNoListLayoutBinding.bind(view))
        }else{
            val view =LayoutInflater.from(parent.context)
    ...
    override fun getItemViewType(position: Int): Int {
        return if(currentList.size==0) EMPTY_VIEW
        else super.getItemViewType(position)
    }

    override fun getItemCount(): Int {
        return if(currentList.size==0)1 else super.getItemCount()
    }
