...
class ContentsDetailFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        binding = FragmentContentsDetailBinding.inflate(inflater, container, false).apply {
            ...
            commentsAdapter = CommentsAdapter { comment, clickType, position ->
                ...
                when (clickType) {
                    ...
                    CommentsClickType.TRANSLATE -> {
                        ...
                        viewModel.startTranslateComment(comment, position)
                    }
                }
            }
            commentsList.run {
...
//                setHasFixedSize(true)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.translate.setOnClickListener {
            viewModel.startTranslateContents(contents)
        }
        ...
        commentsAdapter.addLoadStateListener { loadState ->
            val isEmpty = loadState.append.endOfPaginationReached && commentsAdapter.itemCount < 1
            Timber.d("commentsAdapter.addLoadStateListener : ${loadState.append.endOfPaginationReached} ,${commentsAdapter.itemCount}, isEmpty: $isEmpty")
            if (!isEmpty) {
...
//        }
        ...
        viewModel.updateCommentsAdapter.observe(viewLifecycleOwner) { position ->
            Timber.d("translatedComment position : $position")
            commentsAdapter.notifyItemChanged(position)
        }

        viewModel.translatedContents.observe(viewLifecycleOwner) { item ->
            Timber.d("translatedContents item : $item")
            item.translatedTitle?.let {
                binding.title.text = it
                binding.translate.setColorFilter(requireContext().getColor(R.color.state_active_primary_default))
            } ?: kotlin.run {
                binding.title.text = item.subject
                binding.translate.setColorFilter(requireContext().getColor(R.color.state_active_gray_700))
            }

            item.translatedBody?.let {
...
//        getComments()
    ...
    private fun setSavedIcon(saved: Boolean) {
        ...
        binding.toolbar.menu.getItem(0).icon?.let { DrawableCompat.setTint(it, color) }
    }
