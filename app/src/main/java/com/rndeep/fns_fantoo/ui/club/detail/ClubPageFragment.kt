...
import com.rndeep.fns_fantoo.utils.ConstVariable.ClubDetail.Companion.KEY_JOIN_MEMBER_ID
...
//        //게시글 작성 FAB 버튼
        binding.fabClubPost.setOnClickListener {
            ...
            if(isClubJoin){
                val editorInfo =
                    EditorInfo(EditorType.CLUB, null, clubPageArg.clubId.toString(), myMemberLevel)
    ...
    private fun settingObserver() {
        ...
        clubPageViewModel.clubTopInfoLiveData.observe(this) {
            ...
            setImageWithPlaceHolder(
                ...
                R.drawable.border_radius_8,
                R.drawable.profile_club_character
                )
                ...
                fragmentItem.add(
                    ClubPagePostListType("Home", ClubPageHomeFragment().apply {
                        this.arguments = Bundle().apply {
                            ...
                            putString(KEY_JOIN_MEMBER_ID, myMemberId.toString())
                        }
                    })
                )
            }
...
//            }
    ...
    private fun settingViewTextColors(rgbColor :Int){
        ...
        for (a in 0 until binding.detailToolbar.menu.size) {
            binding.detailToolbar.menu.getItem(a).icon?.colorFilter = menuIconColor
        }
    }
