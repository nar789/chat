...
class DeleteAccountFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModel.myWallet.observe(viewLifecycleOwner) { wallet ->
            wallet?.let {
                binding.myFanitInfo.text =
                    getString(R.string.b_have_fan_it) + " " + wallet.fanit.toString()
                binding.myKdgInfo.text =
                    getString(R.string.b_have_kdg) + " " + wallet.kdg.toString()
                binding.myHonorInfo.text =
                    getString(R.string.b_have_honor) + " " + wallet.honor.toString()
            }
        }
        ...
        viewModel.isWithdrawal.observe(viewLifecycleOwner) {
            if (it) {
                Timber.d("withdrawal is success , go to login page")
                val intent = Intent(requireContext(), LoginMainActivity::class.java)
                intent.putExtra(
                    ConstVariable.INTENT.EXTRA_NAV_START_DESTINATION_ID,
                    R.id.loginFragment
    ...
    private fun openDialog() {
        val message = DialogMessage(
            DialogTitle(
                getString(R.string.s_leave),
                getString(R.string.se_t_sure_want_withdraw),
                null
            ),
