...
@HiltViewModel
class InputVerifyCodeViewModel @Inject constructor(private val registRepository: RegistRepository,
    ...
    fun checkVerifyCode(verifyCode:String, email: String) = viewModelScope.launch {
        when(val response = registRepository.checkVerifyCode(verifyCode, email)){
            ...
            is ResultWrapper.GenericError->{
                _checkingVerifyCodeLiveData.value = VerifyCodeCheckResponse(response.code.toString(), false)
            }
            else -> {}
        }
    }
    ...
    fun requestVerifyCodeToEmail(email: String) = viewModelScope.launch {
        ...
        when(response){
            is ResultWrapper.Success ->{
                _emailVerifyCodeLiveData.value = BaseResponse(ConstVariable.RESULT_SUCCESS_CODE, "", Any())
            }
            is ResultWrapper.NetworkError ->{
                _networkErrorLiveData.value = true
            }
            is ResultWrapper.GenericError ->{
                _emailVerifyCodeLiveData.value = BaseResponse(response.code!!, response.message!!, response.errorData!!)
            }
        }
    }
