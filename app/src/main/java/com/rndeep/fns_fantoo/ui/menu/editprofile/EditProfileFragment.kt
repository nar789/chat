...
class EditProfileFragment : Fragment() {
    ...
    private val pickMedia = registerForActivityResult(ActivityResultContracts.PickVisualMedia()) { uri ->
        uri?.asMultipart("file", "image_${Date().time}", requireContext().contentResolver)
            ?.let { multiPart ->
            ...
            val editProfileAdapter = EditProfileAdapter { item ->
                ...
                when (item.itemType) {
                    ...
                    EditProfileItemType.COUNTRY -> {
                        showCountryBottomSheet(viewModel.getCountry())
                    }
            ...
            editProfileList.run {
                ...
                addItemDecoration(
                    DividerItemDecoration(context, DividerItemDecoration.VERTICAL).apply {
                        AppCompatResources.getDrawable(context, R.drawable.divider_line)
                            ?.let { setDrawable(it) }
                    }
                )
    ...
    private fun showCalendar() {
        ...
        Timber.d("showCalendar, today: $birthday")
        viewModel.userInfo.birthDay?.let {
            birthday = TimeUtils.getEpochMilliTimeFromDate(it)
            Timber.d("showCalendar, birthday: $birthday")
            val timezone = TimeZone.getDefault()
            birthday += timezone.getOffset(birthday)
            Timber.d("showCalendar, add time zone offset birthday: $birthday")
        }
