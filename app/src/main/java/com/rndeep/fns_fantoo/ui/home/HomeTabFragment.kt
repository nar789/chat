...
class HomeTabFragment : BaseFragment<FragmentHomeTabBinding>(FragmentHomeTabBinding::inflate) {
    ...
    fun scrollToUp() {
        when (val currentFragment =
            hometabViewPagerAdapter.getFragment(binding.rcCategoryvp.currentItem)) {
            is HomeFragment -> {
    ...
    private fun initView() {
        ...
        categoryListAdapter.setOnCategoryListener(object :
            ...
            override fun OnCategoryClick(v: View, pos: Int, name: String) {
                currentItem = name
                if (categoryListAdapter.getSelectName() == categoryItems[pos]) return
    ...
    private fun addBadgeOfIcon() {
        val badge = BadgeDrawable.create(requireContext()).apply {
            isVisible = true
            backgroundColor = requireContext().getColor(R.color.gray_25)
            verticalOffset = -25
        }
        BadgeUtils.attachBadgeDrawable(badge, binding.homeToolbar, R.id.menu_alarm)
    }
