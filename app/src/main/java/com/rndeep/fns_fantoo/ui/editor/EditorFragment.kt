...
class EditorFragment : Fragment() {
    ...
    private val pickMultipleImageMedia = registerForActivityResult(
        ActivityResultContracts.PickMultipleVisualMedia(MULTIPLE_MEDIA_IMAGE_COUNT)
    ) { uris ->
        if (uris.isNotEmpty()) {
            Timber.d("Number of items selected: ${uris.size}, image uri : $uris")
            uris.forEach { uri ->
                viewModel.attachImage(uri.toString(), requireContext())
            }
        } else {
    ...
    private val pickMultipleVideoMedia = registerForActivityResult(
        ActivityResultContracts.PickMultipleVisualMedia(MULTIPLE_MEDIA_VIDEO_COUNT)
    ) { uris ->
        if (uris.isNotEmpty()) {
            Timber.d("Number of items selected: ${uris.size}, image uri : $uris")
            uris.forEach { uri ->
                viewModel.attachVideo(uri.toString(), requireContext())
            }
        } else {
            Timber.d("PhotoPicker, No media selected")
        }
    ...
    override fun onCreateView(
        ...
        binding = FragmentEditorBinding.inflate(inflater, container, false).apply {
            ...
            registerBtn.setOnClickListener {
                ...
                if (viewModel.attachedItemFileList.isNotEmpty()) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.inputContent.doOnTextChanged { text, _, _, _ ->
            ...
            val visible = if (text.toString().isNotEmpty()) View.GONE else View.VISIBLE
            binding.inputContentHint.visibility = visible
        }
        ...
        viewModel.modifyCommunityPost.observe(viewLifecycleOwner) { modifyCommunityPost ->
            binding.anonymousSettingsSwitch.isChecked = modifyCommunityPost.anonymYn
            binding.inputTitle.setText(modifyCommunityPost.title)
            binding.inputContent.setText(modifyCommunityPost.content)
        }
        ...
        viewModel.modifyCommunityBoard.observe(viewLifecycleOwner) {
            binding.selectBoard.text = it.codeNameEn
            binding.selectBoard.setTextColor(requireContext().getColor(R.color.btn_inactive_gray))
            binding.selectBoard.isEnabled = false
        }

        viewModel.modifyCommunitySubject.observe(viewLifecycleOwner) {
            binding.selectSubject.text = it.codeNameEn
    ...
    private fun showPhotoPicker() {
        pickMultipleImageMedia.launch(PickVisualMediaRequest(ActivityResultContracts.PickVisualMedia.ImageOnly))
    }

    private fun showVideoPicker() {
        pickMultipleVideoMedia.launch(PickVisualMediaRequest(ActivityResultContracts.PickVisualMedia.VideoOnly))
    }
    ...
    companion object {
        ...
        const val MULTIPLE_MEDIA_IMAGE_COUNT = 5
        const val MULTIPLE_MEDIA_VIDEO_COUNT = 10
    }
