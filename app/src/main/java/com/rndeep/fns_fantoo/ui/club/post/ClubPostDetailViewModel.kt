...
class ClubPostDetailViewModel @Inject constructor(
    ...
    //서버 전송용 이미지 ID 값
    private var attachItem  :List<DetailAttachList>? =null
    fun setAttachItem(id :String){
        attachItem= listOf(
            DetailAttachList("image",id)
        )
    }
    ...
    var attachImageList :List<String>? =null
    ...
    fun getClubPostData(clubId: String, categoryCode: String, postId: Int) {
        viewModelScope.launch {
            val detailResult =
                detailPostRepository.getClubPostDetailItem(
                    accessToken,
                    uId,
                    clubId,
                    categoryCode,
            ...
            if (detailResult.second != null) {
                _detailPostErrorLiveData.value =
                    detailResult.second!!.code ?: ConstVariable.ERROR_WAIT_FOR_SECOND
                ...
                //attachType 0: image , 1: video
                val attachList = arrayListOf<String>()
                postItem.attachList?.forEach{
                    if(it.attachType==0){
                        attachList.add(it.attach)
                    }
                }
                attachImageList=attachList

                if (postItem.deleteType == 1) {
                    _bookMarkExists.value = false
                } else if (postItem.deleteType == 2) {
...
//                _postDetailDataLiveData.value = postData
    ...
    fun getClubPostBookMark(clubId: String, categoryCode: String, postId: Int) {
        viewModelScope.launch {
            if (accessToken != null && uId != null) {
                detailPostRepository.fetchClubPostBookMarkYn(clubId, categoryCode, postId, uId!!)
                    .run {
        ...
        viewModelScope.launch {
            if (isTranslate) {
                ...
                translateRepository.translateWords(
                        ...
                        when (item) {
                            is DetailPostItem.ClubPostDetail -> {
                                originMessage.forEach { transMessage ->
                                    if (item.item.subject == transMessage.origin) item.item.subject =
                                        transMessage.text
                                    if (item.item.content == transMessage.origin) item.item.content =
                                        transMessage.text
                                }
                    ...
                    this.second?.let {
                        _detailPostErrorLiveData.value =
                            it.code ?: ConstVariable.ERROR_WAIT_FOR_SECOND
                    }
                }
            }
        }
    ...
    private fun setChildReplyItem(originalReplyList: List<PostReplyData>?): List<ClubReplyData> {
        ...
        val iterator = replayList.iterator()
        while (iterator.hasNext()) {
            val item = iterator.next()
            if ((item.status == 2 || item.status == 3) && item.childReplyList.isNullOrEmpty()) {
                iterator.remove()
            }
        }
        return replayList
    ...
    fun sendPostComment(clubId: String, categoryCode: String, postId: Int) {
        viewModelScope.launch {
            if (!isLoginUser) {
            ...
            if(inputItem ==null ) {
                _detailPostErrorLiveData.value = ConstVariable.ERROR_C1000
                _resultCommentInput.value = false
                return@launch
            }
...
//                        _detailPostErrorLiveData.value = ConstVariable.ERROR_WAIT_FOR_SECOND
                        _cloudFlareUploadLiveData.value=ERROR_COMMENT_IMAGE_UPLOAD_FAIL
                    } else {
    ...
    fun clickDeleteClubPost(clubId: String, categoryCode: String, postId: Int, reasonMsg: String?) {
        viewModelScope.launch {
            ...
            detailPostRepository.patchDeleteClubPost(clubId, categoryCode, postId, uId!!, reasonMsg)
                .run {
                    if (this.first) {
    ...
    fun requestClubNoticeDetailItem(categoryCode: String, clubId: String, postId: Int) {
        viewModelScope.launch {
            val safeRes = detailPostRepository.getClubNoticeDetailData(
                uId,
                accessToken,
                categoryCode,
                clubId,
...
//                    getClubChallengeCommentList(postId,"init")
    ...
    fun setCommentItem() {
        inputItem = InputCommentItem(
            ...
            if(attachItem.isNullOrEmpty()) null
            else attachItem!![0].id,
            if(attachItem.isNullOrEmpty()) null
            else attachItem!![0].archiveType,
            false,
        )
    }
