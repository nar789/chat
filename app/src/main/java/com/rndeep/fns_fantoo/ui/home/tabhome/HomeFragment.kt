...
class HomeFragment : BaseFragment<FragmentHomeBinding>(FragmentHomeBinding::inflate) {
    ...
    override fun initUiActionEvent() {
        binding.rcHomeList.checkLastItemVisible {
            if (it) {
                //마지막 아이템
                if (!(currentLast) && homeRcAdapter.currentList.size>5) {
...
//            homeViewModel.getInitHomeData(requireContext())
        ...
        homeRcAdapter.setOnRecommendClubClickListener(object :
            ...
            override fun onClubClick(v: View, id: String, name: String) {
                findNavController().navigate(HomeTabFragmentDirections.actionHomeTabFragmentToClubPageDetail(id))
            }
