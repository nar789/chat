...
class CommunityBoardShowAllFragment :
    ...
    fun initView() {
        ...
        viewLifecycleOwner.lifecycleScope.launch{
            val sortType = boardAllViewModel.getFavoritySortType()
            var isSortTypePopular = false
            boardAllViewModel.settingFavoriteItem(
                if(sortType.isNullOrEmpty())ConstVariable.SORTFANTOO else sortType)
            binding.tvSortText.text =
                if(sortType.isNullOrEmpty() || sortType == ConstVariable.SORTFANTOO) {
                    resources.getString(R.string.p_order_by_user_recommend)
                }else{
                    isSortTypePopular = true
                    resources.getString(R.string.a_order_by_popular)
                }
