...
class PopularRepository @Inject constructor(
    ...
    suspend fun getPopularData(
        ...
        if (page == 0) {
            ...
            popularDatas.add(
                BoardPagePosts(
                    ...
                    boardPostShowPosition = ConstVariable.DB_POPULARCATEGORY
                )
            )
        }
        when (res.code) {
        ...
        return Pair(
            boardPagePostDao.getAllBoardPostItemOfShowType(ConstVariable.DB_POPULARCATEGORY),
            null
        )
//        when (safeRes) {
...
//        return postDataDao.getAllItemOfType(ConstVariable.DB_POPULARCATEGORY)
    ...
    suspend fun getRecommendClub(type: String, uid: String?): List<CommonRecommendSelect> {
        var safeRes: ResultWrapper<RecommendationClubResponse>? = null
        when (type) {
            ConstVariable.DB_HOMECATEGORY -> {
                safeRes = safeApiCall(Dispatchers.IO) { homeService.getSuitRecommendClub(uid) }
            }
            ConstVariable.DB_POPULARCATEGORY -> {
                safeRes = safeApiCall(Dispatchers.IO) {
                    if (uid.isNullOrEmpty()) {
                        homeService.getGuestHotRecommendClub()
                    } else {
                        homeService.getHotRecommendClub(uid)
                    }
                }
            }
        }
        ...
        when (safeRes) {
            is ResultWrapper.Success -> {
                ...
                if (recommendClubDao.interItems(recommendClub).isNotEmpty()) {
                    return listOf(
                        recommendClubDao.getRecommendClubAllItemOfType(
                            type
                        )
                    )
                }
