...
@AndroidEntryPoint
class LoginFindPasswordFragment : BaseFragment<FragmentLoginFindPasswordBinding>(FragmentLoginFindPasswordBinding::inflate) {
    private val loginViewModel : LoginViewModel by viewModels()
    override fun initUi() {
        //임시비밀번호 요청 결과
        loginViewModel.requestTempPasswordLiveData.observe(this){
                requestTempPasswordResponse->
            requestTempPasswordResponse?.let {
                when(requestTempPasswordResponse.code){
                    ConstVariable.RESULT_SUCCESS_CODE ->{
                        ...
                        requireActivity().showCustomSnackBar(binding.root, getString(R.string.se_a_sent_cert_number))
