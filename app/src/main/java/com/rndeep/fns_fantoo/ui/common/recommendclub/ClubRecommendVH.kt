...
class ClubRecommendVH(private val binding: CommonRecommendClubBinding,private val categoryClickListener: RecommendClubSortAdapter.RecommendCategoryClickListener?) :
    ...
    fun recommendBind(
        ...
        sortItem: List<ClubRecommendCategory>?,
        ...
        when (item.pageType) {
            ConstVariable.DB_CLUBPAGE_HOT -> {
                if(sortAdapter.itemCount!=0) return
                ...
                sortItem?.let {
                    sortAdapter.setItem(sortItem)
                }
                sortAdapter.setOnCategoryClickListener(categoryClickListener)
