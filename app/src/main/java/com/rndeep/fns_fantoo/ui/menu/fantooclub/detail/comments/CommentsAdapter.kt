...
class CommentsAdapter(
...
) : PagingDataAdapter<FantooClubComment, RecyclerView.ViewHolder>(CommentsDiff) {
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        when(holder) {
            is CommentsViewHolder -> item?.let { holder.bind(it) }
            is CommentsReplyViewHolder -> item?.let { holder.bind(it) }
            is CommentsReportedViewHolder -> item?.let { holder.bind(it) }
            is CommentsDeletedViewHolder -> item?.let { holder.bind(it) }
            is CommentsBlockViewHolder -> item?.let { holder.bind(it) }
        }
    }

    override fun getItemViewType(position: Int) = when(getItem(position)?.let { getViewType(it) }) {
        0 -> TYPE_COMMENT
        ...
        fun bind(item: FantooClubComment) {
            binding.apply {
                ...
                root.setOnClickListener {
                    onItemClicked(item, CommentsClickType.ITEM, bindingAdapterPosition)
                }
