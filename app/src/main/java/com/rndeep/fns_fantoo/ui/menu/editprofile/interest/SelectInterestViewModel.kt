...
class SelectInterestViewModel @Inject constructor(
    ...
    private val _state = MutableStateFlow<SelectInterestUIState>(SelectInterestUIState.None)
    val state = _state.asStateFlow()

    init {
        viewModelScope.launch {
            _state.value = SelectInterestUIState.Loading
    ...
    private fun fetchUserInfo() = viewModelScope.launch {
        ...
        when (response) {
            is ResultWrapper.Success -> {
                ...
                _state.value = SelectInterestUIState.None
            }
            is ResultWrapper.GenericError -> {
                Timber.d("response error code : ${response.code} , server msg : ${response.message} , message : ${response.errorData?.message}")
                _state.value = SelectInterestUIState.Error(Throwable(response.code))
            }
            is ResultWrapper.NetworkError -> {
                // TODO handling network error
                Timber.d("network error")
                _state.value = SelectInterestUIState.Error(Throwable("network error"))
            }
        }
    }
    ...
    fun updateInterest() = viewModelScope.launch {
        ...
        when (response) {
            is ResultWrapper.Success -> {
                ...
                _state.value = SelectInterestUIState.Success(true)
