...
import com.rndeep.fns_fantoo.databinding.FragmentClubSettingBoxCommentBinding
import com.rndeep.fns_fantoo.ui.club.settings.ClubSettingBaseFragment
import com.rndeep.fns_fantoo.ui.club.settings.tabs.adapter.CommentAdapter
import com.rndeep.fns_fantoo.ui.club.settings.tabs.viewmodel.CommentViewModel
...
@AndroidEntryPoint
class CommentFragment(val clubId:String,
                      val uid:String,
                      val memberId:String,
                      val clickListener:(ClubStorageReplyListWithMeta)-> Unit) :
    ClubSettingBaseFragment<FragmentClubSettingBoxCommentBinding>(FragmentClubSettingBoxCommentBinding::inflate) {

    private val commentViewModel : CommentViewModel by viewModels()

    override fun initUi() {
        val adapter = CommentAdapter(){selectedItem: ClubStorageReplyListWithMeta -> clickListener(selectedItem)}
        binding.rv.adapter = adapter
        binding.rv.layoutManager = LinearLayoutManager(context, LinearLayoutManager.VERTICAL, false)

        binding.emptyView.tvEmpty.text = getString(R.string.se_j_no_write_comment)
        ...
        lifecycleScope.launch {
            commentViewModel.getStorageReplyList(clubId, uid, memberId).collectLatest {
        ...
        lifecycleScope.launch {
            adapter.loadStateFlow.distinctUntilChangedBy {
                ...
                setItemDecoreation(adapter.itemCount > 0)
                setListViewVisibility(adapter.itemCount > 0)
            }
        }
    }
