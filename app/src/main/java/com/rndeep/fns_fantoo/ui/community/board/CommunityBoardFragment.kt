...
import com.rndeep.fns_fantoo.utils.ConstVariable.Community.Companion.MAX_REQ_POST_LIST_SIZE
...
class CommunityBoardFragment :
    ...
    override fun initUi() {
        ...
        boardFragmentViewModel.getBoardPostItem(
            ...
            boardSubCode = boardFragmentViewModel.getCurrentBoardCategoryCode(),
            postListSize = getRequestPostListItemCount()
        )
...
//                val res = boardFragmentViewModel.requestHonor(dbId)
            ...
            override fun onProfileClick(postItem: PostListData) {
                if (postItem is BoardPostData) {
                    findNavController().navigate(R.id.action_communityBoardFragment3_to_profile,
                        arguments?.apply {
                            this.putString("nickname", postItem.userNick)
                            this.putBoolean("isBlock", postItem.userBlockYn ?: false)
                            this.putString("targetUid", boardFragmentViewModel.uId)
                            this.putString("userPhoto", postItem.userPhoto ?: "")
                        }
                    )
                }
            }
        ...
        binding.fabBoardPostEdit.setOnClickListener {
            ...
            findNavController().navigate(
                CommunityBoardFragmentDirections.actionCommunityBoardFragment3ToEditorFragment3(
                    editorInfo
                )
            )
        }
    ...
    private fun settingObserver() {
        ...
        boardFragmentViewModel.categoryInfoLiveData.observe(viewLifecycleOwner) { boardInfo ->
            //게시판 제목
            val title = requireContext().getStringByIdentifier(ConstVariable.Community.CategoryCode.compare(boardInfo.first.code).code)
            binding.communityBoardToolbar.title = title
        ...
        activity?.let {
            ...
            bottomSheet.setOnCliCkListener(object : BottomSheetAdapter.OnItemClickListener {
                override fun onItemClick(name: String, pos: Int, oldPos: Int?) {
                    ...
                    if (name != resources.getString(R.string.d_other_language_select)) {
    ...
    fun showLanguageBottomSheet() {
        ...
        lifecycleScope.launch {
            val languageCode = boardFragmentViewModel.getLanguageCode()
        ...
        languageBottomSheet.itemClickListener = (object : AppLanguageBottomSheet.ItemClickListener {
            override fun onItemClick(item: Language) {
                boardFragmentViewModel.setGlobalState(false)
                lifecycleScope.launch {
                    boardFragmentViewModel.setLanguageCode(item.langCode).run {
                        boardFragmentViewModel.getBoardPostItem(
                            getItem.boardInfo.boardId!!,
                            boardFragmentViewModel.getCurrentBoardCategoryCode(),
                            10,
                ...
                for (a in globalSheetData) {
                    a.isChecked =
                        a.itemName == resources.getString(R.string.d_other_language_select)
                }
    ...
    fun clearSelectLangCode() {
        CoroutineScope(Dispatchers.IO).launch {
            boardFragmentViewModel.setLanguageCode(null)
        }
    }

    private fun getRequestPostListItemCount():Int{
        return if (boardPostAdapter.itemCount < 10) {
            10
        } else {
            if ((boardPostAdapter.itemCount - 1 > MAX_REQ_POST_LIST_SIZE)) {
                MAX_REQ_POST_LIST_SIZE
            } else {
                boardPostAdapter.itemCount - 1
            }
        }
    }
}