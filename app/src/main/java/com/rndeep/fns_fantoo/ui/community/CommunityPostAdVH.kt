...
class CommunityPostAdVH(private val binding : CategoryHomePostBinding): RecyclerView.ViewHolder(binding.root){
    ...
    fun postSetting(item : AdInfo?, postType:String){
        ...
        item?.let {
            settingTopView(item.ADImage.ko,item.ADTitle.ko,item.adContents.ko)
            settingContentView(item.ADTitle.ko,item.ADImage.ko)
...
//            .load(thumbnail)
//            .into(binding.ivPostProfileThumbnail)
