...
class ClubPostDetailFragment :
    ...
    override fun initUi() {
        ...
        //영상 전체 화면
        detailPostAdapter.setOnFullScreenClickListener(settingPlayer())
        ...
        detailPostAdapter.setOnPostDetailOptionsClickListener(object :
            ...
            override fun onCategoryTextClick(clubId: String, categoryCode: String) {
                if (navController.previousBackStackEntry?.destination?.id == R.id.clubDetailPageFragment) {
                    findNavController().navigate(
                        R.id.action_clubPostDetailFragment_to_club_detail_page2,
                        arguments.apply {
            ...
            override fun onProfileClick(
                ...
                if (!detailViewModel.isLoginUser) {
        ...
        detailPostAdapter.setOnCommentClickListener(object :
            ...
            override fun onTranslateClick(commentItem: ClubReplyData, pos: Int) {
                commentPos = pos
                detailViewModel.clickTransComment(
                    commentItem.content,
                    pos + 1,
                    pos + 1,
                    commentItem.translateYn ?: false
                )
            }
        ...
        detailPostAdapter.setOnReplyClickListener(object :
    ...
    override fun initUiActionEvent() {
        ...
        binding.edtCommentInput.setOnFocusChangeListener { _, b ->
            if (b && !detailViewModel.isLoginUser) {
    ...
    private fun settingObserver() {
        ...
        detailViewModel.transCommentReply.observe(this) {
            if (detailPostAdapter.currentList[commentPos!!] is DetailPostItem.ClubPostComment) {
                val adapterItem =
                    detailPostAdapter.currentList[commentPos!!] as DetailPostItem.ClubPostComment
                if (commentReplyPos != null) {
                    ...
                    detailPostAdapter.notifyItemChanged(
                        commentPos!!,
...
//                binding.rlCommentContainer.visibility = View.GONE
    ...
    private fun settingModifyMode(
        ...
        if (!attachItem.isNullOrEmpty()) {
            ...
            detailViewModel.setAttachItem(attachItem[0].attach)
        } else {
            binding.clCommentImageContainer.visibility = View.GONE
        }
    }
                    ...
                    when (clubDetailArg.previewType) {
                        ...
                        ConstVariable.TYPE_CLUB_NOTICE -> {
                            getNoticePostSheetItem(
                                postItem.item.memberLevel == CLUB_MEMBERSHIP_LV_MANAGER,
                                detailViewModel.isMember,
                                !detailViewModel.isLoginUser,
                            )
                        }
                        else -> {
                            getPostOptionsText(
                                postItem.item.memberId == clubDetailArg.memberId,
                                postItem.item.memberLevel == CLUB_MEMBERSHIP_LV_MANAGER,
                    ...
                    override fun onItemClick(name: String, pos: Int, oldPos: Int?) {
                        ...
                        when (name) {
                            ...
                            getString(R.string.bottom_sheet_edit) -> {
                                val editorInfo =
                                    EditorInfo(EditorType.CLUB, null, postItem.item.clubId, postItem.item.memberLevel)
                                findNavController().navigate(
                                    ClubPostDetailFragmentDirections.actionClubPostDetailFragmentToEditorFragment(
                                        editorInfo
                                    )
                                )
                            }
        ...
        return if (isGuest) {
            ...
            arrayListOf(
                BottomSheetItem(
                    ...
                    if (detailViewModel.bookmarkClickResultLiveData.value == true)
                        getString(R.string.j_save_cancel)
                    else getString(R.string.j_to_save),
                    null,
                    null
                ),
                BottomSheetItem(
    ...
    fun getChallengePostSheetItem() = arrayListOf(
        BottomSheetItem(R.drawable.icon_outline_share, getString(R.string.g_to_share), null, null),
    )
    ...
    private fun settingPlayer() = object :
        ...
        override fun onImageClick(pos: Int) {
            if(!detailViewModel.attachImageList.isNullOrEmpty()){
                val sendAttachList :Array<String> = detailViewModel.attachImageList!!.toTypedArray()
                findNavController().navigate(R.id.action_clubPostDetailFragment_to_imageListViewerFragment,
                    arguments.apply {
