...
class InviteFriendFragment : Fragment() {
            ...
            viewModel.codeState.collect { state ->
                when(state) {
                    is RegisterCodeState.Success -> {
                        ...
                        if (state.codeState == RecommendCodeState.USERINFO) {
                            val visible = viewModel.checkVisibleCodeInput()
                            Timber.d("visible : $visible")
                            setVisibleInputCode(visible)
                        } else {
    ...
    private fun showErrorMessage(code: String?) {
        ...
        val message = DialogMessage(
            DialogTitle(getString(R.string.c_invite_friend), msg, null),
            DialogButton(getString(R.string.h_confirm), null, false),
            isCompleted = false
        )
    ...
    private fun showMessage(state: RecommendCodeState) {
        var message = DialogMessage(
            DialogTitle(getString(R.string.c_invite_friend), getString(R.string.se_a_write_referral_code), null),
            DialogButton(getString(R.string.h_confirm), null, false),
            isCompleted = false
        )
        dialogFragment = when (state) {
            ...
            RecommendCodeState.VALID -> {
                message = DialogMessage(
                    DialogTitle(getString(R.string.c_invite_friend), getString(R.string.se_c_referral_code_succeess_registered), String.format(getString(R.string.se_p_fanit_paid_and_check), "500")),
                    DialogButton(getString(R.string.h_confirm), null, false),
                    isCompleted = false
                )
                MenuDialogFragment(message) { dismissDialog() }
            ...
            RecommendCodeState.INVALID -> {
                message = DialogMessage(
                    DialogTitle(getString(R.string.c_invite_friend), getString(R.string.se_a_invalid_referral_code), null),
                    DialogButton(getString(R.string.h_confirm), null, false),
                    isCompleted = false
                )
                MenuDialogFragment(message) { dismissDialog() }
            ...
            RecommendCodeState.MYSELF -> {
                message = DialogMessage(
                    DialogTitle(getString(R.string.c_invite_friend), getString(R.string.se_b_cannot_use_your_code), null),
                    DialogButton(getString(R.string.h_confirm), null, false),
                    isCompleted = false
                )
                MenuDialogFragment(message) { dismissDialog() }
                ...
                message = DialogMessage(
                    DialogTitle(getString(R.string.c_invite_friend), "else", null),
                    DialogButton(getString(R.string.h_confirm), null, false),
                    isCompleted = false
                )
                MenuDialogFragment(message) { dismissDialog() }
    ...
    private fun setVisibleInputCode(isVisible: Boolean) {
        val visible = if (isVisible) View.VISIBLE else View.GONE
        binding.recommenderCodeInputTitle.visibility = visible
        binding.recommenderCodeInputLayout.visibility = visible
        binding.codeRegisterBtn.visibility = visible
        binding.recommendInputDesc.visibility = visible
    }
}