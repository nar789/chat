...
class EditorViewModel @Inject constructor(
    ...
    private val attachedItemList = mutableListOf<MultimediaItem>()
    ...
    val communityAttaches: MutableList<Attach> = mutableListOf()
    ...
    fun composePost() {
        when(editorInfo.editorType) {
            ...
            EditorType.COMMUNITY -> composeCommunityPost()
        }
    }
    ...
    private fun composeCommunityPost() = viewModelScope.launch {
        setHashtags()
        val response =
            communityRepository.composeCommunityPost(accessToken, composeCommunityPost, _board.value)
        Timber.d("responseData : $response")
        when (response) {
            is ResultWrapper.Success -> {
    ...
    private fun fetchClubTwoDepthCategories(clubId: String, categoryCode: String) = viewModelScope.launch {
        val response = clubRepository.fetchClubTwoDepthCategories(clubId, categoryCode, integUid)
        Timber.d("responseData : $response")
        when (response) {
            is ResultWrapper.Success -> {
    ...
    fun deleteAttachedItem(item: MultimediaItem) {
        Timber.d("deleteAttachedItem , remove item : $item")
                ...
                MultimediaType.IMAGE -> {
                    uploadRepository.fileUploadImageToCloudFlare(cloudKey, file).run {
                        if (this == null) {
                            ...
                            when(editorInfo.editorType) {
                                EditorType.CLUB -> clubAttaches.add(AttachClub(this, CLUB_ATTACH_IMAGE, null))
                                EditorType.COMMUNITY -> communityAttaches.add(Attach(COMMUNITY_ATTACH_IMAGE, this))
                            }
                        }
                    }
                }
                MultimediaType.VIDEO -> {
                    uploadRepository.tusVideoUpload(preferences,sendFile!!,cloudKey,"VIDEO_${Date().time}").run {
                        if (this == null) {
                            cloudFlareIDList.add(CloudFlareItem(false, position, null, fileType))
                        } else {
                            cloudFlareIDList.add(CloudFlareItem(true, position, this, fileType))
                            when(editorInfo.editorType) {
                                EditorType.CLUB -> clubAttaches.add(AttachClub(this, CLUB_ATTACH_VIDEO, null))
                                EditorType.COMMUNITY -> communityAttaches.add(Attach(COMMUNITY_ATTACH_VIDEO, this))
                            }
                        }
                    }
//                    uploadRepository.fileUploadVideoToCloudFlare(cloudKey, file).run {
//                        if (this == null) {
//                            cloudFlareIDList.add(CloudFlareItem(false, position, null, fileType))
//                        } else {
//                            cloudFlareIDList.add(CloudFlareItem(true, position, this, fileType))
